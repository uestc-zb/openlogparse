...
namespace OpenLogReplicator {
    Metadata::Metadata(Ctx *newCtx, Locales *newLocales, std::string newDatabase, typeConId newConId, Scn newStartScn,
                       Seq newStartSequence, std::string newStartTime,
                       uint64_t newStartTimeRel) : schema(new Schema(newCtx, newLocales)),
                                                   ctx(newCtx),
                                                   locales(newLocales),
    ...
    void Metadata::setSeqFileOffset(Seq newSequence, FileOffset newFileOffset) {
        if (unlikely(ctx->isTraceSet(Ctx::TRACE::CHECKPOINT)))
            ctx->logTrace(Ctx::TRACE::CHECKPOINT,
                          "setting sequence to: " + newSequence.toString() + ", offset: " + newFileOffset.toString());
    ...
    SchemaElement *Metadata::addElement(const std::string &owner, const std::string &table, DbTable::OPTIONS options1,
                                        DbTable::OPTIONS options2) {
        return addElement(owner, table,
                          static_cast<DbTable::OPTIONS>(static_cast<uint>(options1) | static_cast<uint>(options2)));
    }
    ...
    SchemaElement *Metadata::addElement(const std::string &owner, const std::string &table, DbTable::OPTIONS options) {
        if (unlikely(!Data::checkNameCase(owner)))
            throw ConfigurationException(
                30003, "owner '" + owner + "' contains lower case characters, value must be upper case");
        if (unlikely(!Data::checkNameCase(table)))
            throw ConfigurationException(
                30004, "table '" + table + "' contains lower case characters, value must be upper case");
    ...
    void Metadata::resetElements() {
        ...
        static const std::vector<std::string> sysTables{
            "DEFERRED_STG\\$", "LOB\\$", "LOBCOMPPART\\$", "LOBFRAG\\$", "OBJ\\$", "TAB\\$", "TABPART\\$",
    ...
    void Metadata::buildMaps(std::vector<std::string> &msgs, std::unordered_map<typeObj, std::string> &tablesUpdated) {
        for (const SchemaElement *element: schemaElements) {
            if (ctx->isLogLevelAt(Ctx::LOG::DEBUG))
                msgs.push_back(
                    "- creating table schema for owner: " + element->owner + " table: " + element->table + " options: "
                    +
                    std::to_string(static_cast<uint>(element->options)));

            schema->buildMaps(element->owner, element->table, element->keyList, element->key, element->tagType,
                              element->tagList, element->tag,
                              element->condition, element->options, tablesUpdated, suppLogDbPrimary, suppLogDbAll,
                              defaultCharacterMapId,
    ...
    void Metadata::checkpoint(Thread *t, Scn newCheckpointScn, Time newCheckpointTime, Seq newCheckpointSequence,
                              FileOffset newCheckpointFileOffset,
                              uint64_t newCheckpointBytes, Seq newMinSequence, FileOffset newMinFileOffset,
                              Xid newMinXid) { {
            t->contextSet(Thread::CONTEXT::CHKPT, Thread::REASON::CHKPT);
            std::unique_lock<std::mutex> const lck(mtxCheckpoint);
    ...
    void Metadata::writeCheckpoint(Thread *t, bool force) {
        ...
        std::ostringstream ss; {
            ...
            if (lastSequence == sequence && !force &&
                (static_cast<uint64_t>(checkpointTime.toEpoch(ctx->hostTimezone) - lastCheckpointTime.
                                       toEpoch(ctx->hostTimezone)) < ctx->checkpointIntervalS) &&
        ...
        if (unlikely(ctx->isTraceSet(Ctx::TRACE::CHECKPOINT)))
            ctx->logTrace(Ctx::TRACE::CHECKPOINT, "write scn: " + lastCheckpointScn.toString() + " time: " +
                                                  std::to_string(lastCheckpointTime.getVal()) + " seq: " + lastSequence.
                                                  toString() + " offset: " +
