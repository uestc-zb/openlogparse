        ...
        try {
            while (!ctx->hardShutdown) {
                ...
                if (ctx->softShutdown && ctx->replicatorFinished)
                    break;
                if (ctx->configUpdated) {
                    ctx->configUpdated = false;
                    delete[] configFileBuffer;
                    // 从内存配置创建缓冲区
                    configFileBuffer = new char[ctx->config.length() + 1];
                    strcpy(configFileBuffer, ctx->config.c_str());
                    configFileBuffer[ctx->config.length()] = 0;
                    try {
                        updateConfigFile();
                        ctx->info(0, "Configuration updated via API");
                    } catch (const std::exception& ex) {
                        ctx->error(0, "Failed to update configuration: " + std::string(ex.what()));
                    }
