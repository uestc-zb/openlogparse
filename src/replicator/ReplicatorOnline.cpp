...
namespace OpenLogReplicator {
    ...
    void ReplicatorOnline::positionReader() {
        ...
        if (!metadata->startTime.empty()) {
            ...
            // NOW
            std::string sql = sqlGetDatabaseScn();
            DatabaseStatement stmt(conn);
            if (unlikely(ctx->isTraceSet(Ctx::TRACE::SQL)))
        ...
        if (metadata->startSequence != Seq::none()) {
            ...
            std::string sql;
            if (standby) {
                sql = sqlGetSequenceFromScnStandby();
                if (unlikely(ctx->isTraceSet(Ctx::TRACE::SQL))) {
                    ctx->logTrace(Ctx::TRACE::SQL, std::string(sql));
    ...
    std::string ReplicatorOnline::getParameterValue(std::string parameter) const {
        ...
        DatabaseStatement stmt(conn);
        std::string sql = sqlGetParameter();
        if (unlikely(ctx->isTraceSet(Ctx::TRACE::SQL))) {
            ctx->logTrace(Ctx::TRACE::SQL, std::string(sql));
    ...
    void ReplicatorOnline::updateOnlineRedoLogData() {
        ...
        {
            std::string sql = sqlGetDatabaseRole();
            DatabaseStatement stmt(conn);
            if (unlikely(ctx->isTraceSet(Ctx::TRACE::SQL)))
                ctx->logTrace(Ctx::TRACE::SQL, std::string(sql));
            stmt.createStatement(sql);
        ...
        {
            std::string sql = sqlGetDatabaseIncarnation();
            DatabaseStatement stmt(conn);
            if (unlikely(ctx->isTraceSet(Ctx::TRACE::SQL)))
                ctx->logTrace(Ctx::TRACE::SQL, std::string(sql));
            stmt.createStatement(sql);
        ...
        {
            DatabaseStatement stmt(conn);
            std::string sql = sqlGetLogfileList();
            if (unlikely(ctx->isTraceSet(Ctx::TRACE::SQL))) {
                ctx->logTrace(Ctx::TRACE::SQL, std::string(sql));
    ...
    std::string ReplicatorOnline::sqlGetArchiveLogList() {
        return std::string(SQL_GET_ARCHIVE_LOG_LIST);
    }

    std::string ReplicatorOnline::sqlGetDatabaseInformation() {
        return std::string(SQL_GET_DATABASE_INFORMATION);
    }

    std::string ReplicatorOnline::sqlGetDatabaseIncarnation() {
        return std::string(SQL_GET_DATABASE_INCARNATION);
    }

    std::string ReplicatorOnline::sqlGetDatabaseRole() {
        return std::string(SQL_GET_DATABASE_ROLE);
    }

    std::string ReplicatorOnline::sqlGetDatabaseScn() {
        return std::string(SQL_GET_DATABASE_SCN);
    }

    std::string ReplicatorOnline::sqlGetSequenceFromScn() {
        return std::string(SQL_GET_SEQUENCE_FROM_SCN);
    }

    std::string ReplicatorOnline::sqlGetSequenceFromScnStandby() {
        return std::string(SQL_GET_SEQUENCE_FROM_SCN_STANDBY);
    }

    std::string ReplicatorOnline::sqlGetLogfileList() {
        return std::string(SQL_GET_LOGFILE_LIST);
    }

    std::string ReplicatorOnline::sqlGetParameter() const{
        return std::string(SQL_GET_PARAMETER);
    }

    void ReplicatorOnline::updateDatabaseIncarnation() {
        if (!checkConnection()) {
            ctx->warning(60001, "database connection not available for incarnation update");
            return;
        }

        if (unlikely(ctx->isTraceSet(Ctx::TRACE::SQL)))
            ctx->logTrace(Ctx::TRACE::SQL, "updating database incarnation information");

        contextSet(CONTEXT::CHKPT, REASON::CHKPT);
        std::unique_lock<std::mutex> const lck(metadata->mtxCheckpoint);

        // 保存旧的 resetlogs 值用于比较
        typeResetlogs const oldResetlogs = metadata->resetlogs;
        ...
        try {
            ...
            while (ret != 0) {
                ...
                if (unlikely(ctx->isTraceSet(Ctx::TRACE::SQL)))
                    ctx->logTrace(Ctx::TRACE::SQL, "found incarnation: " + std::to_string(incarnation) +
                                ", resetlogs: " + std::to_string(resetlogs) +
                                ", status: " + std::string(status.data()));
                ...
                if (oldResetlogs != 0 && oi->resetlogs == oldResetlogs) {
                    ...
                    foundCurrent = true;
                    if (unlikely(ctx->isTraceSet(Ctx::TRACE::SQL)))
                        ctx->logTrace(Ctx::TRACE::SQL, "restored previous incarnation: " + std::to_string(oi->resetlogs));
                } else if (oi->current && metadata->dbIncarnationCurrent == nullptr) {
                    // 使用数据库标记为当前的 incarnation
                    metadata->dbIncarnationCurrent = oi;
                    metadata->setResetlogs(oi->resetlogs);
                    foundCurrent = true;

                    if (oldResetlogs != oi->resetlogs) {
                        ctx->info(0, "updated resetlogs_id from " + std::to_string(oldResetlogs) +
                                    " to " + std::to_string(oi->resetlogs));
                    }

                    if (unlikely(ctx->isTraceSet(Ctx::TRACE::SQL)))
                        ctx->logTrace(Ctx::TRACE::SQL, "set current incarnation: " + std::to_string(oi->resetlogs));
                }

                ret = stmt.next();
            }

            // 验证是否找到了当前 incarnation
            if (!foundCurrent) {
                ctx->warning(60002, "no current incarnation found in database");

                // 如果没有找到当前的，尝试使用第一个可用的
                if (!metadata->dbIncarnations.empty()) {
                    auto* firstIncarnation = *metadata->dbIncarnations.begin();
                    metadata->dbIncarnationCurrent = firstIncarnation;
                    metadata->setResetlogs(firstIncarnation->resetlogs);
                    ctx->warning(60003, "using first available incarnation: " + std::to_string(firstIncarnation->resetlogs));
                }
            }

            if (metadata->dbIncarnationCurrent != nullptr) {
                ctx->info(0, "database incarnation updated successfully, current resetlogs_id: " +
                            std::to_string(metadata->dbIncarnationCurrent->resetlogs));
            } else {
                ctx->error(60004, "failed to set current database incarnation");
            }

        } catch (const std::exception& e) {
            ctx->error(60005, "failed to update database incarnation: " + std::string(e.what()));

            // 在出错时，如果 oldResetlogs 不为 0，尝试恢复
            if (oldResetlogs != 0) {
                metadata->setResetlogs(oldResetlogs);
                ctx->warning(60006, "restored previous resetlogs_id: " + std::to_string(oldResetlogs));
            }
        }
