...
#include "writer/RacWriterFile.h"
#include "writer/RacMergeWriterFile.h"


#ifdef LINK_LIBRARY_OCI
#include "replicator/ReplicatorOnline.h"
#include "replicator/ReplicatorRacOnline.h"
...
namespace OpenLogReplicator {
    ...
    OpenLogReplicator::OpenLogReplicator(std::string newConfigFileName, Ctx *newCtx) : configFileName(
            std::move(newConfigFileName)),
        ctx(newCtx) {
        IntX::initializeBASE10();
    }

    OpenLogReplicator::OpenLogReplicator(const char *configFileBuffer, std::string newConfigFileName,
                                         Ctx *newCtx) : configFileName(WEB_CONFIG_FILE_NAME),
                                                        ctx(newCtx) {
        this->configFileBuffer = new char[strlen(configFileBuffer) + 1]; // �������ڴ�
        strcpy(this->configFileBuffer, configFileBuffer);
        IntX::initializeBASE10();
    }
    ...
    void OpenLogReplicator::readConfigFile(struct stat &configFileStat) {
        if (configFileName == WEB_CONFIG_FILE_NAME) {
            configFileStat.st_mtime = -1;
            return;
        }
        ...
        if (configFileStat.st_size > Checkpoint::CONFIG_FILE_MAX_SIZE || configFileStat.st_size == 0)
            throw ConfigurationException(
                10004, "file: " + configFileName + " - wrong size: " + std::to_string(configFileStat.st_size));
        ...
        if (bytesRead != static_cast<uint64_t>(configFileStat.st_size))
            throw RuntimeException(
                10005, "file: " + configFileName + " - " + std::to_string(bytesRead) + " bytes read instead of " +
                       std::to_string(configFileStat.st_size));
        configFileBuffer[configFileStat.st_size] = 0;
    }

    void OpenLogReplicator::do_work(int instId, Locales *locales, struct stat configFileStat, const rapidjson::Value &sourceJson, const std::string alias, uint64_t memoryMaxMb) {
        ...
        if (!ctx->isDisableChecksSet(Ctx::DISABLE_CHECKS::JSON_TAGS)) {
            static const std::vector<std::string> readerNames{
                ...
                "server", "redo-log", "path-mapping", "log-archive-format", "asm"
            };
        ...
        if (sourceJson.HasMember("flags")) {
            ...
            if (ctx->flags > 524287)
                throw ConfigurationException(
                    30001, "bad JSON, invalid \"flags\" value: " + std::to_string(ctx->flags) +
        ...
        if (readerJson.HasMember("disable-checks")) {
            ...
            if (ctx->disableChecks > 15)
                throw ConfigurationException(30001, "bad JSON, invalid \"disable-checks\" value: " +
                                                    std::to_string(ctx->disableChecks) +
        ...
        if (readerJson.HasMember("start-seq")) {
            const rapidjson::Value &startScnJson = Ctx::getJsonFieldO(configFileName, readerJson, "start-seq");
            if (startScnJson.HasMember(std::to_string(instId).c_str())) {
                startSequence = Ctx::getJsonFieldU32(configFileName, startScnJson, std::to_string(instId).c_str());
            } else {
        ...
        if (readerJson.HasMember("start-time-rel")) {
            ...
            if (startScn != Scn::none())
                throw ConfigurationException(
                    30001, "bad JSON, invalid \"start-time-rel\" value: " + std::to_string(startTimeRel) +
        ...
        if (readerJson.HasMember("start-time")) {
            ...
            if (startScn != Scn::none())
                throw ConfigurationException(30001, "bad JSON, invalid \"start-time\" value: " + startTime +
                                                    ", expected: unset when \"start-scn\" is set (" + startScn.
                                                    toString() + ")");
            if (startTimeRel > 0)
                throw ConfigurationException(30001, "bad JSON, invalid \"start-time\" value: " + startTime +
                                                    ", expected: unset when \"start-time-rel\" is set (" +
                                                    std::to_string(startTimeRel) + ")");
        }
        ...
        if (sourceJson.HasMember("state")) {
            ...
            if (stateJson.HasMember("type")) {
                const std::string stateTypeStr = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH,
                                                                    stateJson, "type");
                if (stateTypeStr == "disk") {
                    ...
                    throw ConfigurationException(
                        30001,
                        std::string("bad JSON, invalid \"type\" value: ") + stateTypeStr +
        ...
        if (sourceJson.HasMember("debug")) {
            ...
            if (!ctx->isFlagSet(Ctx::REDO_FLAGS::SCHEMALESS) && (
                    debugJson.HasMember("owner") || debugJson.HasMember("table"))) {
        ...
        if (sourceJson.HasMember("transaction-max-mb")) {
            const uint64_t transactionMaxMb =
                    Ctx::getJsonFieldU64(configFileName, sourceJson, "transaction-max-mb");
            if (transactionMaxMb > memoryMaxMb)
                throw ConfigurationException(
                    30001, "bad JSON, invalid \"transaction-max-mb\" value: " + std::to_string(transactionMaxMb) +
        ...
        if (sourceJson.HasMember("metrics")) {
            ...
            if (metricsJson.HasMember("type")) {
                const std::string metricsType = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH,
                                                                   metricsJson, "type");
                Metrics::TAG_NAMES tagNames = Metrics::TAG_NAMES::NONE;

                if (metricsJson.HasMember("tag-names")) {
                    const std::string tagNamesStr = Ctx::getJsonFieldS(
                        configFileName, Ctx::JSON_TOPIC_LENGTH, metricsJson, "tag-names");
                    ...
                    if (tagNamesStr == "none")
                        ...
                        throw ConfigurationException(
                            30001, "bad JSON, invalid \"tag-names\" value: " + tagNamesStr +
                                   R"(, expected: one of {"all", "filter", "none", "sys"})");
                }

                if (metricsType == "prometheus") {
#ifdef LINK_LIBRARY_PROMETHEUS
                    const std::string prometheusBind = Ctx::getJsonFieldS(
                        configFileName, Ctx::JSON_TOPIC_LENGTH, metricsJson, "bind");
                    ...
                    throw ConfigurationException(
                        30001,
                        R"(bad JSON, invalid "type" value: ")" + metricsType +
        ...
        // CHECKPOINT
        auto *checkpoint = new Checkpoint(ctx, metadata, alias + "-checkpoint", configFileName,
        ...
        if (formatJson.HasMember("db")) {
            ...
            if (val > 3)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"db\" value: " + std::to_string(val) + ", expected: one of {0 .. 3}");
        ...
        if (formatJson.HasMember("attributes")) {
            ...
            if (val > 7)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"attributes\" value: " + std::to_string(val) +
                    ", expected: one of {0 .. 7}");
        ...
        if (formatJson.HasMember("interval-dts")) {
            ...
            if (val > 10)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"interval-dts\" value: " + std::to_string(val) +
        ...
        if (formatJson.HasMember("interval-ytm")) {
            ...
            if (val > 4)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"interval-ytm\" value: " + std::to_string(val) +
                    ", expected: one of {0 .. 4}");
        ...
        if (formatJson.HasMember("message")) {
            ...
            if (val > 31)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"message\" value: " + std::to_string(val) + ", expected: one of {0 .. 31}");
            if ((val & static_cast<uint>(Format::MESSAGE_FORMAT::FULL)) != 0 &&
                ...
                throw ConfigurationException(30001, "bad JSON, invalid \"message\" value: " + std::to_string(val) +
                                                    ...
                                                    std::to_string(
                                                        static_cast<uint>(Format::MESSAGE_FORMAT::SKIP_BEGIN)) +
                                                    "/" + std::to_string(
                                                        static_cast<uint>(Format::MESSAGE_FORMAT::SKIP_COMMIT)) +
        ...
        if (formatJson.HasMember("rid")) {
            ...
            if (val > 1)
                throw ConfigurationException(
                    30001, "bad JSON, invalid \"rid\" value: " + std::to_string(val) + ", expected: one of {0, 1}");
        ...
        if (formatJson.HasMember("xid")) {
            ...
            if (val > 2)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"xid\" value: " + std::to_string(val) + ", expected: one of {0 .. 2}");
        ...
        if (formatJson.HasMember("timestamp")) {
            ...
            if (val > 15)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"timestamp\" value: " + std::to_string(val) +
                    ", expected: one of {0 .. 15}");
        ...
        if (formatJson.HasMember("timestamp-tz")) {
            ...
            if (val > 11)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"timestamp-tz\" value: " + std::to_string(val) +
        ...
        if (formatJson.HasMember("timestamp-all")) {
            ...
            if (val > 1)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"timestamp-all\" value: " + std::to_string(val) +
                    ", expected: one of {0, 1}");
        ...
        if (formatJson.HasMember("char")) {
            ...
            if (val > 3)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"char\" value: " + std::to_string(val) + ", expected: one of {0 .. 3}");
        ...
        if (formatJson.HasMember("scn")) {
            ...
            if (val > 1)
                throw ConfigurationException(
                    30001, "bad JSON, invalid \"scn\" value: " + std::to_string(val) + ", expected: one of {0, 1}");
        ...
        if (formatJson.HasMember("scn-type")) {
            ...
            if (val > 3)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"scn-type\" value: " + std::to_string(val) + ", expected: one of {0, 3}");
        ...
        if (formatJson.HasMember("unknown")) {
            ...
            if (val > 1)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"unknown\" value: " + std::to_string(val) + ", expected: one of {0, 1}");
        ...
        if (formatJson.HasMember("schema")) {
            ...
            if (val > 7)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"schema\" value: " + std::to_string(val) + ", expected: one of {0 .. 7}");
        ...
        if (formatJson.HasMember("column")) {
            ...
            if (val > 2)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"column\" value: " + std::to_string(val) + ", expected: one of {0 .. 2}");
            ...
            if (ctx->isFlagSet(Ctx::REDO_FLAGS::SCHEMALESS) && val != 0)
                throw ConfigurationException(30001, "bad JSON, invalid \"column\" value: " + std::to_string(val) +
                                                    ", expected: not used when flags has set schemaless mode (flags: "
        ...
        if (formatJson.HasMember("unknown-type")) {
            ...
            if (val > 1)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"unknown-type\" value: " + std::to_string(val) +
                    ", expected: one of {0, 1}");
        ...
        const std::string formatType = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH, formatJson,
                                                          "type");

        Builder *builder;
        Format format(dbFormat, attributesFormat, intervalDtsFormat, intervalYtmFormat, messageFormat, ridFormat,
                      xidFormat, timestampFormat,
                      timestampTzFormat, timestampAll, charFormat, scnFormat, scnType, unknownFormat, schemaFormat,
                      columnFormat, unknownType);
        if (formatType == "json") {
            ...
            throw ConfigurationException(
                30001, "bad JSON, invalid \"format\" value: " + formatType + R"(, expected: "protobuf" or "json")");
        builders.push_back(builder);

        // READER
        const std::string readerType = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH, readerJson,
                                                          "type");
        ...
        if (sourceJson.HasMember("arch-read-tries")) {
            ...
            if (ctx->archReadTries < 1 || ctx->archReadTries > 1000000000)
                throw ConfigurationException(30001, "bad JSON, invalid \"arch-read-tries\" value: " +
                                                    std::to_string(ctx->archReadTries) +
        ...
        if (readerJson.HasMember("redo-copy-path"))
            ctx->redoCopyPath = Ctx::getJsonFieldS(configFileName, Ctx::MAX_PATH_LENGTH, readerJson,
                                                   "redo-copy-path");

        if (readerJson.HasMember("db-timezone")) {
            const std::string dbTimezone = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH,
                                                              readerJson, "db-timezone");
            if (!Data::parseTimezone(dbTimezone, ctx->dbTimezone))
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"db-timezone\" value: " + dbTimezone + ", expected value: {\"+/-HH:MM\"}");
        }

        if (readerJson.HasMember("host-timezone")) {
            const std::string hostTimezone = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH,
                                                                readerJson, "host-timezone");
            if (!Data::parseTimezone(hostTimezone, ctx->hostTimezone))
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"host-timezone\" value: " + hostTimezone +
                    ", expected value: {\"+/-HH:MM\"}");
        }

        if (readerJson.HasMember("log-timezone")) {
            const std::string logTimezone = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH,
                                                               readerJson, "log-timezone");
            if (!Data::parseTimezone(logTimezone, ctx->logTimezone))
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"log-timezone\" value: " + logTimezone +
                    ", expected value: {\"+/-HH:MM\"}");
        }

        if (readerType == "online") {
#ifdef LINK_LIBRARY_OCI
            const std::string user = Ctx::getJsonFieldS(configFileName, Ctx::JSON_USERNAME_LENGTH, readerJson,
                                                        "user");
            const std::string password = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PASSWORD_LENGTH, readerJson,
                                                            "password");
            const std::string server = Ctx::getJsonFieldS(configFileName, Ctx::JSON_SERVER_LENGTH, readerJson,
                                                          "server");
            bool keepConnection = false;

            if (sourceJson.HasMember("arch")) {
                const std::string arch = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH, sourceJson,
                                                            "arch");
                ...
                if (arch == "path")
                    ...
                    if (instId == -1) {
                        ...
                        archGetLog = ReplicatorRacOnline::archGetLogOnline;
                    }
            ...
            if (instId != -1) {
                replicator = new ReplicatorRacOnline(instId, ctx, archGetLog, builder, metadata, transactionBuffer, alias, name,
                                                     user, password, server, keepConnection);
                if (readerJson.HasMember("asm")) {
                    dynamic_cast<ReplicatorRacOnline*>(replicator)->setAsm(true);
                }
            } else
                replicator = new ReplicatorOnline(ctx, archGetLog, builder, metadata, transactionBuffer, alias, name,
                                                  user, password, server, keepConnection);
            builder->initialize();
            ...
            const rapidjson::Value &redoLogBatchArrayJson = Ctx::getJsonFieldA(
                configFileName, readerJson, "redo-log");

            for (rapidjson::SizeType k = 0; k < redoLogBatchArrayJson.Size(); ++k)
                replicator->addRedoLogsBatch(
                    Ctx::getJsonFieldS(configFileName, Ctx::MAX_PATH_LENGTH, redoLogBatchArrayJson, "redo-log", k));
        } else
            throw ConfigurationException(
                30001,
                "bad JSON, invalid \"type\" value: " + readerType +
        ...
        if (sourceJson.HasMember("filter")) {
            ...
            if (filterJson.HasMember("separator"))
                separator = Ctx::getJsonFieldS(configFileName, Ctx::JSON_FORMAT_SEPARATOR_LENGTH, filterJson,
                                               "separator");
            ...
            if (filterJson.HasMember("table") && !ctx->isFlagSet(Ctx::REDO_FLAGS::SCHEMALESS)) {
                ...
                for (rapidjson::SizeType k = 0; k < tableArrayJson.Size(); ++k) {
                    const rapidjson::Value &tableElementJson = Ctx::getJsonFieldO(
                        configFileName, tableArrayJson, "table", k);

                    if (!ctx->isDisableChecksSet(Ctx::DISABLE_CHECKS::JSON_TAGS)) {
                        static const std::vector<std::string> tableElementNames{
                            "owner", "table", "key", "condition", "tag"
                        };
                        Ctx::checkJsonFields(configFileName, tableElementJson, tableElementNames);
                    }

                    const std::string owner = Ctx::getJsonFieldS(configFileName, SysUser::NAME_LENGTH,
                                                                 tableElementJson, "owner");
                    const std::string table = Ctx::getJsonFieldS(configFileName, SysObj::NAME_LENGTH,
                                                                 tableElementJson, "table");
                    ...
                    if (tableElementJson.HasMember("key")) {
                        element->key = Ctx::getJsonFieldS(configFileName, Ctx::JSON_KEY_LENGTH, tableElementJson,
                                                          "key");
                        element->parseKey(element->key, separator);
                    };

                    if (tableElementJson.HasMember("condition"))
                        element->condition = Ctx::getJsonFieldS(configFileName, Ctx::JSON_CONDITION_LENGTH,
                                                                tableElementJson, "condition");

                    if (tableElementJson.HasMember("tag")) {
                        element->tag = Ctx::getJsonFieldS(configFileName, Ctx::JSON_TAG_LENGTH, tableElementJson,
                                                          "tag");
            ...
            if (filterJson.HasMember("skip-xid")) {
                const rapidjson::Value &skipXidArrayJson = Ctx::getJsonFieldA(
                    configFileName, filterJson, "skip-xid");
                for (rapidjson::SizeType k = 0; k < skipXidArrayJson.Size(); ++k) {
                    const Xid xid(Ctx::getJsonFieldS(configFileName, Ctx::JSON_XID_LENGTH, skipXidArrayJson,
                                                     "skip-xid", k));
            ...
            if (filterJson.HasMember("dump-xid")) {
                const rapidjson::Value &dumpXidArrayJson = Ctx::getJsonFieldA(
                    configFileName, filterJson, "dump-xid");
                for (rapidjson::SizeType k = 0; k < dumpXidArrayJson.Size(); ++k) {
                    const Xid xid(Ctx::getJsonFieldS(configFileName, Ctx::JSON_XID_LENGTH, dumpXidArrayJson,
                                                     "dump-xid", k));
        ...
        if (readerJson.HasMember("log-archive-format")) {
            ...
            replicator->metadata->logArchiveFormat = Ctx::getJsonFieldS(
                configFileName, DbTable::VPARAMETER_LENGTH, readerJson, "log-archive-format");
        }
    ...
    Writer* OpenLogReplicator::createWriter(Replicator *replicator2, const rapidjson::Value &targetJson) {
        const std::string alias = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH, targetJson,
                                                     "alias");
        const std::string source = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH, targetJson,
                                                      "source");
        ...
        const std::string writerType = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH, writerJson,
                                                          "type");
        ...
        if (writerJson.HasMember("poll-interval-us")) {
            ...
            if (ctx->pollIntervalUs < 100 || ctx->pollIntervalUs > 3600000000)
                throw ConfigurationException(30001, "bad JSON, invalid \"poll-interval-us\" value: " +
                                                    std::to_string(ctx->pollIntervalUs) +
        ...
        if (writerJson.HasMember("queue-size")) {
            ...
            if (ctx->queueSize < 1 || ctx->queueSize > 1000000)
                throw ConfigurationException(
                    30001, "bad JSON, invalid \"queue-size\" value: " + std::to_string(ctx->queueSize) +
        ...
        if (writerType == "file") {
            ...
            if (writerJson.HasMember("timestamp-format"))
                timestampFormat = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH, writerJson,
                                                     "timestamp-format");
            ...
            if (writerJson.HasMember("output"))
                ...
                throw ConfigurationException(30001, "bad JSON, invalid \"output\" value: " + output +
                                                    ", expected: to be set when \"max-file-size\" is set (" +
            ...
            if (writerJson.HasMember("new-line")) {
                ...
                if (newLine > 2)
                    throw ConfigurationException(
                        30001,
                        "bad JSON, invalid \"new-line\" value: " + std::to_string(newLine) +
                        ", expected: one of {0 .. 2}");
            }
            ...
            if (writerJson.HasMember("append")) {
                ...
                if (append > 1)
                    throw ConfigurationException(
                        30001,
                        "bad JSON, invalid \"append\" value: " + std::to_string(append) +
                        ", expected: one of {0, 1}");
            }
            ...
            if (writerJson.HasMember("write-buffer-flush-size")) {
                ...
                if (writeBufferFlushSize > 1048576)
                    throw ConfigurationException(30001, "bad JSON, invalid \"write-buffer-flush-size\" value: " +
                                                        std::to_string(writeBufferFlushSize) +
                                                        ", expected: one of {0 .. 1048576}");
            }
            if (replicator2 == nullptr) {
                 writer = new RacMergeWriterFile(ctx, alias + "-writer", "", nullptr,
                                         nullptr, output, timestampFormat,
                                         maxFileSize, newLine, append, writeBufferFlushSize);
            }else {
                if(typeid(*replicator2)==typeid(ReplicatorRacOnline))
                    writer = new RacWriterFile(ctx, alias + "-writer", replicator2->database, replicator2->builder,
                                        replicator2->metadata, output, timestampFormat,
                                        maxFileSize, newLine, append, writeBufferFlushSize);
                else
                    writer = new RacWriterFile(ctx, alias + "-writer", replicator2->database, replicator2->builder,
                                        replicator2->metadata, output, timestampFormat,
            ...
            writer = new WriterDiscard(ctx, alias + "-writer", replicator2->database, replicator2->builder,
                                       replicator2->metadata);
            ...
            if (writerJson.HasMember("max-message-mb")) {
                ...
                if (maxMessageMb < 1 || maxMessageMb > WriterKafka::MAX_KAFKA_MESSAGE_MB)
                    throw ConfigurationException(
                        30001, "bad JSON, invalid \"max-message-mb\" value: " + std::to_string(maxMessageMb) +
                               ", expected: one of {1 .. " + std::to_string(WriterKafka::MAX_KAFKA_MESSAGE_MB) +
                               "}");
            }
            replicator2->builder->setMaxMessageMb(maxMessageMb);

            const std::string topic = Ctx::getJsonFieldS(configFileName, Ctx::JSON_TOPIC_LENGTH, writerJson,
                                                         "topic");

            writer = new WriterKafka(ctx, alias + "-writer", replicator2->database, replicator2->builder,
                                     replicator2->metadata, topic);

            if (writerJson.HasMember("properties")) {
                const rapidjson::Value &propertiesJson = Ctx::getJsonFieldO(
                    configFileName, writerJson, "properties");

                for (rapidjson::Value::ConstMemberIterator itr = propertiesJson.MemberBegin();
                     itr != propertiesJson.MemberEnd(); ++itr) {
...
#ifdef LINK_LIBRARY_PROTOBUF
            const std::string uri = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH, writerJson,
                                                       "uri");

            auto *stream = new StreamNetwork(ctx, uri);
            stream->initialize();
            writer = new WriterStream(ctx, alias + "-writer", replicator2->database, replicator2->builder,
    ...
    int OpenLogReplicator::run() {
        ...
        readConfigFile(configFileStat);

        rapidjson::Document document;
        if (document.Parse(configFileBuffer).HasParseError())
            throw DataException(
                20001, "file: " + configFileName + " offset: " + std::to_string(document.GetErrorOffset()) +
                       " - parse error: " + GetParseError_En(document.GetParseError()));

        if (!ctx->isDisableChecksSet(Ctx::DISABLE_CHECKS::JSON_TAGS)) {
            static const std::vector<std::string> documentNames{
                "version", "dump-path", "dump-raw-data", "dump-redo-log", "log-level", "trace", "source",
                "target"
            };
        ...
        if (document.HasMember("dump-redo-log")) {
            ...
            if (ctx->dumpRedoLog > 2)
                throw ConfigurationException(
                    30001, "bad JSON, invalid \"dump-redo-log\" value: " + std::to_string(ctx->dumpRedoLog) +
                           ", expected: one of {0 .. 2}");

            if (ctx->dumpRedoLog > 0) {
                if (document.HasMember("dump-path"))
                    ctx->dumpPath = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH, document,
                                                       "dump-path");
        ...
        if (document.HasMember("log-level")) {
            ...
            if (ctx->logLevel > Ctx::LOG::DEBUG)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"log-level\" value: " + std::to_string(static_cast<uint>(ctx->logLevel)) +
                    ", expected: one of {0 .. 4}");
        }
        ...
        if (sourceArrayJson.Size() != 1) {
            throw ConfigurationException(
                30001, "bad JSON, invalid \"source\" value: " + std::to_string(sourceArrayJson.Size()) +
        ...
        for (rapidjson::SizeType j = 0; j < sourceArrayJson.Size(); ++j) {
            ...
            if (!ctx->isDisableChecksSet(Ctx::DISABLE_CHECKS::JSON_TAGS)) {
                static const std::vector<std::string> sourceNames{
                    ...
                    "redo-verify-delay-us", "refresh-interval-us", "arch", "filter","rac"
                };
                Ctx::checkJsonFields(configFileName, sourceJson, sourceNames);
            }

            const std::string alias = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH, sourceJson,
                                                         "alias");
            ...
            if (sourceJson.HasMember("memory")) {
                ...
                if (memoryJson.HasMember("min-mb")) {
                    ...
                    if (memoryMinMb < Ctx::MEMORY_CHUNK_MIN_MB)
                        throw ConfigurationException(
                            30001, "bad JSON, invalid \"min-mb\" value: " + std::to_string(memoryMinMb) +
                ...
                if (memoryJson.HasMember("max-mb")) {
                    ...
                    if (memoryMaxMb < memoryMinMb)
                        throw ConfigurationException(
                            30001, "bad JSON, invalid \"max-mb\" value: " + std::to_string(memoryMaxMb) +
                ...
                if (memoryJson.HasMember("unswap-buffer-min-mb")) {
                    ...
                    memoryUnswapBufferMinMb = (memoryUnswapBufferMinMb / Ctx::MEMORY_CHUNK_SIZE_MB) *
                                              Ctx::MEMORY_CHUNK_SIZE_MB;
                }
                ...
                if (memoryJson.HasMember("swap-mb")) {
                    ...
                    if (memorySwapMb > memoryMaxMb - 4)
                        throw ConfigurationException(
                            30001, "bad JSON, invalid \"swap-mb\" value: " + std::to_string(memorySwapMb) +
                ...
                if (memoryJson.HasMember("read-buffer-min-mb")) {
                    ...
                    memoryReadBufferMinMb = (memoryReadBufferMinMb / Ctx::MEMORY_CHUNK_SIZE_MB) *
                                            Ctx::MEMORY_CHUNK_SIZE_MB;
                    if (memoryReadBufferMinMb > memoryMaxMb)
                        throw ConfigurationException(30001, "bad JSON, invalid \"read-buffer-min-mb\" value: " +
                                                            ...
                                                            ", expected: not greater than \"max-mb\" value (" +
                ...
                if (memoryJson.HasMember("read-buffer-max-mb")) {
                    ...
                    memoryReadBufferMaxMb = (memoryReadBufferMaxMb / Ctx::MEMORY_CHUNK_SIZE_MB) *
                                            Ctx::MEMORY_CHUNK_SIZE_MB;
                    ...
                    if (memoryReadBufferMaxMb < memoryReadBufferMinMb)
                        throw ConfigurationException(30001, "bad JSON, invalid \"read-buffer-max-mb\" value: " +
                                                            std::to_string(memoryReadBufferMaxMb) +
                                                            ", expected: at least: \"read-buffer-min-mb\" value (" +
                ...
                if (memoryJson.HasMember("write-buffer-min-mb")) {
                    ...
                    memoryWriteBufferMinMb = (memoryWriteBufferMinMb / Ctx::MEMORY_CHUNK_SIZE_MB) *
                                             Ctx::MEMORY_CHUNK_SIZE_MB;
                ...
                if (memoryJson.HasMember("write-buffer-max-mb")) {
                    ...
                    memoryWriteBufferMaxMb = (memoryWriteBufferMaxMb / Ctx::MEMORY_CHUNK_SIZE_MB) *
                                             Ctx::MEMORY_CHUNK_SIZE_MB;
                    ...
                    if (memoryWriteBufferMaxMb < memoryWriteBufferMinMb)
                        throw ConfigurationException(30001, "bad JSON, invalid \"write-buffer-max-mb\" value: " +
                                                            std::to_string(memoryWriteBufferMaxMb) +
                                                            ", expected: at least: \"write-buffer-min-mb\" value (" +
                                                            std::to_string(memoryWriteBufferMinMb) + ")");
                }

                if (memoryJson.HasMember("swap-path") && memorySwapMb > 0)
                    memorySwapPath = Ctx::getJsonFieldS(configFileName, Ctx::JSON_PARAMETER_LENGTH, memoryJson,
                                                        "swap-path");

                if (memoryUnswapBufferMinMb + memoryReadBufferMinMb + memoryWriteBufferMinMb + 4 > memoryMaxMb)
                    throw ConfigurationException(
                        30001,
                        R"(bad JSON, invalid "unswap-buffer-min-mb" + "read-buffer-min-mb" + "write-buffer-min-mb" + 4 ()"
                        +
            ...
            // MEMORY MANAGER
            ctx->initialize(memoryMinMb, memoryMaxMb, memoryReadBufferMaxMb, memoryReadBufferMinMb, memorySwapMb,
                            memoryUnswapBufferMinMb,
            ...
            if (sourceJson.HasMember("rac")) {
                // do_work(1, locales, configFileStat, sourceJson, alias, memoryMaxMb);
                do_work(2, locales, configFileStat, sourceJson, alias, memoryMaxMb);
            }else {
                do_work(-1, locales, configFileStat, sourceJson, alias, memoryMaxMb);
            }
        }
        ...
        if (targetArrayJson.Size() != 1) {
            throw ConfigurationException(
                30001, "bad JSON, invalid \"target\" value: " + std::to_string(targetArrayJson.Size()) +
                       " elements, expected: 1 element");
        }
        ...
        for (rapidjson::SizeType j = 0; j < targetArrayJson.Size(); ++j) {
            ...
            Writer *mergeWriter = createWriter(nullptr, targetJson);
            for (Replicator *replicatorTmp: replicators)
                if (replicatorTmp->alias == source) {
                    replicator2 = replicatorTmp;
                    Writer *writer = createWriter(replicator2, targetJson);
                    // if (typeid(writer) == typeid(RacWriterFile)) {
                        RacWriterFile *racWriterFile = reinterpret_cast<RacWriterFile *>(writer);
                        racWriterFile->setRacMergeWriterFile(reinterpret_cast<RacMergeWriterFile *>(mergeWriter));
                    // }
            ...
            if (replicator2 == nullptr)
                throw ConfigurationException(
                    30001,
                    "bad JSON, invalid \"source\" value: " + source +
    ...
    void OpenLogReplicator::mainProcessMapping(const rapidjson::Value &readerJson) {
        if (readerJson.HasMember("path-mapping")) {
            const rapidjson::Value &pathMappingArrayJson = Ctx::getJsonFieldA(
                configFileName, readerJson, "path-mapping");

            if ((pathMappingArrayJson.Size() % 2) != 0)
                throw ConfigurationException(30001, "bad JSON, invalid \"path-mapping\" value: " +
                                                    std::to_string(pathMappingArrayJson.Size()) +
                                                    " elements, expected: even number of elements");

            for (rapidjson::SizeType k = 0; k < pathMappingArrayJson.Size() / 2; ++k) {
                const std::string sourceMapping = Ctx::getJsonFieldS(configFileName, Ctx::MAX_PATH_LENGTH,
                                                                     pathMappingArrayJson, "path-mapping", k * 2);
                const std::string targetMapping = Ctx::getJsonFieldS(configFileName, Ctx::MAX_PATH_LENGTH,
                                                                     pathMappingArrayJson, "path-mapping", (k * 2) + 1);
