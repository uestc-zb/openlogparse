...
namespace OpenLogReplicator {
    WriterFile::WriterFile(Ctx *newCtx, std::string newAlias, std::string newDatabase, Builder *newBuilder,
                           Metadata *newMetadata,
                           std::string newOutput, std::string newTimestampFormat, uint64_t newMaxFileSize,
                           uint64_t newNewLine, uint64_t newAppend,
                           uint newWriteBufferFlushSize) : Writer(newCtx, std::move(newAlias), std::move(newDatabase),
    ...
    void WriterFile::initialize() {
        ...
        if ((prefixPos = fileNameMask.find("%i")) != std::string::npos) {
            ...
            if (append == 0)
                throw ConfigurationException(
                    30006, "output file is with no rotation: " + this->output + " - 'append' must be set to 1");
            mode = MODE::NO_ROTATE;
        }
        ...
        if (mode == MODE::NUM) {
            ...
            while ((ent = readdir(dir)) != nullptr) {
                ...
                if (fileName.length() < suffix.length() || fileName.substr(fileName.length() - suffix.length()) !=
                    suffix)
                    continue;

                if (unlikely(ctx->isTraceSet(Ctx::TRACE::WRITER)))
                    ctx->logTrace(Ctx::TRACE::WRITER, "found previous output file: " + pathName + "/" + fileName);
                const std::string fileNameFoundNum(
                    fileName.substr(prefix.length(), fileName.length() - suffix.length() - prefix.length()));
                uint64_t fileNum;
                try {
    ...
    std::string WriterFile::getNextFullFileName() {
        ...
        return pathName + "/" + fileNameMask.substr(0, prefixPos) + std::string(zeros, '0') + outputFileNumStr +
    ...
    void WriterFile::checkFile(Scn scn __attribute__((unused)), Seq sequence, uint64_t size) {
        ...
        if (mode == MODE::NO_ROTATE) {
            ...
            if (fileSize + size > maxFileSize) {
                ++fileNameNum;
                std::string endMsg = R"({"nextFileName":")" + getNextFullFileName() + "\"}\n";
                bufferedWrite(reinterpret_cast<const uint8_t *>(endMsg.c_str()), endMsg.length());
                flush();
                closeFile();
                fileSize = 0;
            }
            ...
            if (outputDes == -1) {
                fullFileName = std::move(getNextFullFileName());
            }
            ...
            if (outputDes == -1 || shouldSwitch) {
                ...
                const std::string newOutputFile =
                        pathName + "/" + fileNameMask.substr(0, prefixPos) + str + fileNameMask.substr(suffixPos);
        ...
        if (outputDes == -1) {
            ...
            if (statRet == 0) {
                ...
                if (append == 0)
                    throw RuntimeException(
                        10003, "file: " + fullFileName + " - get metadata returned: " + strerror(errno));
            ...
            if (outputDes == -1)
                throw RuntimeException(
                    10006, "file: " + fullFileName + " - open for writing returned: " + strerror(errno));
    ...
    void WriterFile::unbufferedWrite(const uint8_t *data, uint64_t size) {
        ...
        if (bytesWritten <= 0 || static_cast<uint64_t>(bytesWritten) != size)
            throw RuntimeException(
                10007, "file: " + fullFileName + " - " + std::to_string(bytesWritten) + " bytes written instead of " +
